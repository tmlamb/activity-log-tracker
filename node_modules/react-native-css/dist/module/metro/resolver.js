...
export function nativeResolver(resolver, context, moduleName, platform) {
  ...
  const isReactNativeIndex = context.originModulePath.endsWith(`${sep}react-native${sep}index.js`);
...
export function webResolver(resolver, context, moduleName, platform) {
  ...
  if (!isIndex || !module || !allowedModules.has(module) || module === "VirtualizedList") {
    return resolution;
  }
  return resolver(context, `react-native-css/components/${module}`, platform);
}
