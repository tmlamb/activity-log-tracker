 ...
 */
export function getSelectors(selectorList, isDarkMode, builder, selectors = []) {
  for (let cssSelector of selectorList) {
    ...
    isDarkMode && isDefaultVariableSelector(cssSelector)) {
      selectors.push({
...
function classNameSelector(selector, options) {
      ...
      case "attribute":
        {
          ...
          const attributeQuery = component.name.startsWith("data-") ? ["d", toRNProperty(component.name.replace("data-", ""))] : ["a", toRNProperty(component.name)];
...
function isIsPseudoClass(selector) {
  return selector.length === 1 && selector[0]?.type === "pseudo-class" && selector[0].kind === "is";
}
function isWherePseudoClass(selector) {
  return selector.length === 1 && selector[0]?.type === "pseudo-class" && selector[0].kind === "where";
}
...
// Matches:  .dark:root  {}
function isRootDarkVariableSelector([first, second]) {
  return first && second && (
  // .dark:root {}
  first.type === "class" && second.type === "pseudo-class" && second.kind === "root" ||
  // :root[class~=dark] {}
  first.type === "pseudo-class" && first.kind === "root" && second.type === "attribute" && second.name === "class" && second.operation && ["includes", "equal"].includes(second.operation.operator));
}
