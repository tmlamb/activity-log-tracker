...
const getSortedNodes = (nodes, options) => {
    const { importOrder, importOrderCombineTypeAndValueImports, hasAnyCustomGroupSeparatorsInImportOrder, provideGapAfterTopOfFileComments, importOrderSafeSideEffects, } = options;
    ...
    const splitBySideEffectNodes = nodes.reduce((chunks, node) => {
        const type = (0, get_chunk_type_of_node_1.getChunkTypeOfNode)(node, importOrderSafeSideEffects);
