...
export function createShareable(hostRuntimeId, initial, config) {
  ...
  const shareableRef = WorkletsModule.createShareable(UIRuntimeId, createSerializable(initial), !!initSynchronously, createSerializable(hostDecorator), createSerializable(guestDecorator));
  return globalThis.__shareableGuestUnpacker(UIRuntimeId, shareableRef, guestDecorator);
}
